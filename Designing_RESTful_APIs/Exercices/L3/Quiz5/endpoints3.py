...
def puppiesFunction():
    if request.method == 'GET':
        #Call the method to get all puppies
        return getAllPuppies()
    elif request.method == 'POST':
        print("Make a New Puppy")
...
def updatePuppy(id,name,description):
    ...
    session.add(puppy)
    session.commit()
    return "Updated a Puppy with id {}".format(id)

def deletePuppy(id):
    puppy = session.query(Puppies).filter_by(id = id).one()
    session.delete(puppy)
    session.commit()
    return "Removed Puppy wit id {}".format(id)
